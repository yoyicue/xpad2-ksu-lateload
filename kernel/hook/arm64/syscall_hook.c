...
void ksu_syscall_table_hook(int nr, syscall_fn_t fn, syscall_fn_t *old)
{
    ...
    /* Never track an entry that was not actually modified.  Besides making
     * the state truthful, this prevents module exit from "restoring" a
     * failed hook through the same broken write path. */
