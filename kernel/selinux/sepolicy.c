...
static bool remove_avtab_node(struct policydb *db, struct avtab_node *node)
{
    ...
    /*
     * Linux 4.19 stores avtab buckets in a flex_array.  The late-load base
     * policy only adds allow rules, so it never needs to prune a node.  Keep
     * the modern removal path out rather than interpreting the flex_array as
     * a pointer table and corrupting the live policydb.
     */
    return false;
#else
    int i;
...
static bool add_type(struct policydb *db, const char *type_name, bool attr)
{
    ...
    /* Linux 4.19 policydb indexes types through fixed-size flex_arrays. */
    new_type_attr_map_array = flex_array_alloc(sizeof(struct ebitmap), value, GFP_ATOMIC | __GFP_ZERO);
    new_type_val_to_struct = flex_array_alloc(sizeof(struct type_datum *), value, GFP_ATOMIC | __GFP_ZERO);
    new_val_to_name_types = flex_array_alloc(sizeof(char *), value, GFP_ATOMIC | __GFP_ZERO);
    ...
    if (flex_array_prealloc(new_type_attr_map_array, 0, value, GFP_ATOMIC | __GFP_ZERO) ||
        flex_array_prealloc(new_type_val_to_struct, 0, value, GFP_ATOMIC | __GFP_ZERO) ||
        flex_array_prealloc(new_val_to_name_types, 0, value, GFP_ATOMIC | __GFP_ZERO)) {
    ...
    for (i = 0; i < db->type_attr_map_array->total_nr_elements; i++) {
        ...
        if (old_elem && flex_array_put(new_type_attr_map_array, i, old_elem, GFP_ATOMIC | __GFP_ZERO))
    ...
    for (i = 0; i < db->type_val_to_struct_array->total_nr_elements; i++) {
        ...
        if (old_elem && flex_array_put_ptr(new_type_val_to_struct, i, old_elem, GFP_ATOMIC | __GFP_ZERO))
            goto legacy_copy_fail;
    }
    for (i = 0; i < value - 1; i++) {
        old_elem = flex_array_get_ptr(db->sym_val_to_name[SYM_TYPES], i);
        if (old_elem && flex_array_put_ptr(new_val_to_name_types, i, old_elem, GFP_ATOMIC | __GFP_ZERO))
            goto legacy_copy_fail;
    }
    ...
    if (ebitmap_set_bit(flex_array_get(db->type_attr_map_array, value - 1), value - 1, 1))
        return false;

    old_fa = db->type_val_to_struct_array;
    db->type_val_to_struct_array = new_type_val_to_struct;
    flex_array_free(old_fa);
    if (flex_array_put_ptr(db->type_val_to_struct_array, value - 1, type, GFP_ATOMIC | __GFP_ZERO))
        return false;

    old_fa = db->sym_val_to_name[SYM_TYPES];
    db->sym_val_to_name[SYM_TYPES] = new_val_to_name_types;
    flex_array_free(old_fa);
    if (flex_array_put_ptr(db->sym_val_to_name[SYM_TYPES], value - 1, key, GFP_ATOMIC | __GFP_ZERO))
        return false;
