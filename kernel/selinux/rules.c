...
static struct policydb *clone_legacy_policydb(struct selinux_state *state)
{
    ...
    /* policydb_write() omits these Android vendor flags on 4.19. */
...
void apply_kernelsu_rules()
{
...
#ifdef CONFIG_KSU_LEGACY_4_19
    if (!ksu_type(db, KERNEL_SU_DOMAIN, "domain") || !ksu_permissive(db, KERNEL_SU_DOMAIN)) {
...
#ifdef CONFIG_KSU_LEGACY_4_19
    /* Keep selinuxfs policy reads safely overprovisioned for the new symbols. */
    db->len += PAGE_SIZE;
    ...
    /* Ownership of the cloned policydb internals moved into ss->policydb. */
    kfree(db);
